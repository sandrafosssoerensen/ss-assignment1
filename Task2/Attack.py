...
def build_grade_cookie(msg_bytes: bytes, sig_bytes: bytes) -> str:
    ...
    return json_to_cookie(json.dumps(payload))
...
def find_blinded_message(n: int, e: int, target_msg: bytes) -> tuple[int, bytes]:
    k = (n.bit_length() + 7) // 8
...
# c = m^e mod n
# c = ciphertext, m = message, e = exponent and n = prime number
# Can use the fact that textbook RSA is mallabale: 
# RSA Blind Signatures
def blind(msg, pubkey):
    """Create a blinded version of msg using the public key."""
    modulus_n = pubkey[1]
    r = blindingfactor(modulus_n)
    m = int(msg)
    blinded_msg = (pow(r, *pubkey) * m) % modulus_n
    print("Blinded Message " + str(blinded_msg))
    return r

# RSA Unblind Signatures
def unblind(msg, r, pubkey):
    """Reverse blinding using r and the public key."""
    modulus_n = pubkey[1]
    blinded_sig = int(msg)
    unblinded_sig = (blinded_sig * modinv(r, modulus_n)) % modulus_n
    print("Unblinded Signed Message " + str(unblinded_sig))
...
def main():
    ...
    r, candidate_bytes = find_blinded_message(n, e, target_msg)
